...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        #self.dwconv = nn.Conv2d(dim, dim, kernel_size=(1,3), padding=(0,1), groups=dim) # depthwise conv
    ...
    def forward(self, x):
        input = x
        x = self.dwconv(x)
        x = x.permute(0, 2, 3, 1) # (N, C, H, L) -> (N, H, L, C)
        x = self.norm(x)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.pwconv2(x)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 1, 2) # (N, H, L, C) -> (N, C, H, L)
        #print(self.training)
...
class ConvNeXt(nn.Module):
    ...
    """
    def __init__(self, stack_num=16, frame_skip=1,
                 depths=[3, 3, 18, 3], 
                 dims=[96, 192, 384, 768], 
                 drop_path_rate=0., 
                 drop_prob = 0.2, #0.2,
        ...
        stem = nn.Sequential(
            #nn.Conv2d(in_chans, dims[0], kernel_size=(1,3), stride= (1,1), padding = (0, 1)),
            nn.Conv2d(in_chans, dims[0], kernel_size=(1,3), stride= (1,3)),
    ...
    def forward(self, input_tensor, targets=None, init_check = False): #tensor_list: NestedTensor):
        ...
        x1 = x.clone()
        x1 = rearrange(x1, 'b ch st ft -> b (st ft) ch')
        ...
        x2 = rearrange(x2, 'b ch st ft -> b (st ft) ch')
