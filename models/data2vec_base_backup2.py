...
class Data2Vec_base(nn.Module):
    ...
    def __init__(self, in_chans=3,
                 ...
                 scale_level = 3, attentional_pooling = True, scale_order = 'small_first'):
        super().__init__()
        ...
        dummy = torch.zeros((64, 64, 6, 768)) # batch channel slowtime fasttime
    ...
    def forward_online_encoder(self, x, init_check = False):
        # Input : embedded x
    ...
    def forward_online_transformer_decoder(self, x, memory, init_check=False) :
        for i in range(self.decoder_stages) :
            memory = self.single_modal_transformer_decoders[i](x[len(x)-(i+1+1)], memory)
            if init_check :
    ...
    def forward_vc_decoder(self, RF_vec, init_check=False) :
        ...
        vc_query = torch.zeros((b, 16*16, ch)).to(device)
        vc_query_pos = self.VC_query_pos_embed(vc_query).to(device)
    ...
    def forward_imgencoder(self, x, mask_ratio = 0.25, init_check = False):
        ...
        img_pos_emb = self.pos_embed(masked_imgfeat).detach()
        masked_imgfeat = masked_imgfeat + img_pos_emb.to(device)
        
        #cls imgfeat embedding
        cls_token = torch.zeros((b, 1, ch)).requires_grad_(requires_grad=True).to(device)
    ...
    def forward(self, x, img_feat, mask_ratio=0.25, init_check = False):
        ...
        if self.task == 'selfsupervised' :
            online_latent = self.forward_online_encoder(full_x[len(x)-1], init_check) #smallest feat
            online_RF_vec = self.forward_online_transformer_decoder(full_x, online_latent, init_check) ### RF vec
            ...
            if self.attentional_pooling :
                #online_latent = self.forward_online_encoder(full_x[len(x)-1], mask, init_check)
                online_latent = self.forward_online_encoder(full_x[len(x)-1], init_check)
                online_RF_vec = self.forward_online_transformer_decoder(full_x, online_latent, init_check) ### RF vec
            else :
                online_RF_vec = self.forward_online_encoder(full_x, init_check)
            VC, vc_emb = self.forward_vc_decoder(online_RF_vec, init_check)
            online_outputs = {"online_vc": VC, 
...
class pose_model_petr(nn.Module):
    def __init__(self, embed_dim = 256, num_heads = 8, people_queries = 15, multi_modal_depth = 2, pose_dec_depth = 2,device = None, use_vc = False, feature_gt = False, use_only_gt = False):
        ...
        vc_encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=num_heads, dim_feedforward=256*4,  batch_first=True, activation="gelu")
        self.vc_attention = nn.TransformerEncoder(vc_encoder_layer, num_layers=2)
        person_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=num_heads, dim_feedforward=256*4, batch_first=True, activation="gelu")
        if not self.use_vc :
            self.multimodal_decoder = nn.TransformerDecoder(person_decoder_layer, num_layers=multi_modal_depth)
        self.pos = PositionalEncoding1D(256)
        self.person_decoder = nn.TransformerDecoder(person_decoder_layer, num_layers=pose_dec_depth)
        self.person_pos_embed = PositionalEncoding1D(256)
        
        if self.embed_dim != self.pose_embed_dim :
            self.PtoP_MLP = nn.Linear(embed_dim, 256, bias=False)
        
        pose_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=16, dim_feedforward=256*4, batch_first=True, activation="gelu")
        self.pose_decoder = nn.TransformerDecoder(pose_decoder_layer, num_layers=1)
        self.pose_pos_embed = PositionalEncoding1D(256)
        ...
        self.forward(vc, init_check=True)
        
    def forward(self, visual_clue, init_check=False) :
        device = visual_clue.device
        
        if self.feature_gt :
            if self.embed_dim != 256 :
                vc = self.gt_MLP(visual_clue)
            else :
                vc = visual_clue
            ...
            vc_pos = self.pos(visual_clue).detach().to(device)
            vc = visual_clue + vc_pos
            vc = self.vc_attention(vc)
        ...
        person_queries = torch.zeros((b, self.people_queries, ch), requires_grad=True).to(device)
        pos_encoding = self.person_pos_embed(person_queries).detach().to(device)
        if init_check  == False :
        ...
        for person_num in range(person_queries) :
            pose_queries = torch.zeros((b, self.keypoint_num+1, 256), requires_grad=True).to(device) #class + keypoints
            pose_pos_encoding = self.pose_pos_embed(pose_queries).to(device)
...
def build_model(args):   
    ...
    model = Data2Vec_base(
        ...
        task = args.task, scale_level = args.multi_scale, attentional_pooling = args.attentional_pooling, scale_order = args.scale_order)
